...
function validatePackageSync(workdir: string): string[] {
  ...
  const rootPkgPath = resolve(workdir, "aggrid/package.json")
  const distPkgPath = resolve(workdir, "aggrid/projects/nggrids/package.json")
          ...
          `Commit validation failed:\n${allErrors.map((e) => `  - ${e}`).join("\n")}\n\n` +
            (requireJiraKey
              ...
              : `Expected format: <short description> [ai]\n` +
                `Example: fix power grid column resize [ai]`)
        )
      }
    },
  }
