...
You are a **migration assistant** for the Servoy NG Grids project. Your job is to
...
The user provides:
- A component name (e.g., `powergrid`, `datagrid`) OR `all` for a full scan
- Optionally a specific migration type (e.g., "migrate powergrid to signals")
...
1. `aggrid/projects/nggrids/src/<name>/<name>.ts` — the component class
2. `aggrid/projects/nggrids/src/<name>/<name>.html` — the template
3. `aggrid/projects/nggrids/src/<name>/<name>.service.ts` — the service (if present)
...
- **Handlers are inputs, not outputs** — in Servoy components, event handlers like
  `onCellClick` are received as callback functions via `input<>()`, not emitted.
...
- **Don't touch legacy files** — only modify files under
  `projects/nggrids/src/`. The AngularJS files in `datasettable/` and `groupingtable/`
  stay unchanged.
- **Service files too** — the service (`.service.ts`) may also need signal migration
  if it reads component inputs directly.
