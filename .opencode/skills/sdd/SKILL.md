...
3. **Post via curl.** Use the Jira REST API (see `JIRA.md` for authentication):

   ```bash
   TOKEN="$ATLASSIAN_AUTH_BASIC"
   JSON_BODY='<ADF json string>'
   curl -s -X POST \
...
When approved, use `git status` to see all changes. Stage every file
belonging to this feature with `git add`.

**Include:** feature code, spec file, test files, modified package.json (if changed)
**Exclude:** opencode.json, .opencode/, unrelated files, node_modules/

Commit with `git commit`. Message format:
```
