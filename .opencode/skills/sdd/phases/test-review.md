...
Use `grep` and `glob` to locate test files (`.spec.ts`) related to the
...
- [ ] Uses `TestBed.configureTestingModule` with proper declarations
- [ ] Uses `NO_ERRORS_SCHEMA` to suppress unknown directive warnings
- [ ] Does NOT import `NGGridsModule`
...
**AG Grid specifics**
- [ ] Grid API interactions tested through proper channels
- [ ] Grid ready state properly handled before assertions
- [ ] Column/row operations verified through AG Grid API or DOM
