...
| Non-functional requirements | Performance, backward compatibility? |
| UI/UX | If the component touches visual rendering, is the expected behaviour described? |
...
Use search tools (`grep`, `glob`) to understand the relevant parts of the codebase:
- Find existing implementations of similar features or components
- Understand the dual-layer architecture:
  - Servoy spec file (`aggrid/datasettable/datasettable.spec` or `aggrid/groupingtable/groupingtable.spec`)
  - Angular implementation (`aggrid/projects/nggrids/src/powergrid/` or `aggrid/projects/nggrids/src/datagrid/`)
  - AngularJS implementation (`aggrid/datasettable/datasettable.js` or `aggrid/groupingtable/groupingtable.js`)
- Identify patterns in existing components that should be followed
- Check `@servoy/public` base classes for inherited behaviour
- Understand the service pattern (`powergrid.service.ts` / `datagrid.service.ts`)
...
```bash
git blame -L <start>,<end> "aggrid/<file-path>"
```
...
Example: `docs/SVY-22100-powergrid-column-resize.spec.md`
...
<Ordered list of the concrete changes needed — files to create/modify, properties
to add to the .spec, Angular component changes, template changes, etc.>
...
```
docs/SVY-22100-powergrid-column-resize.spec.md
```
