...
Use search tools (`grep`, `glob`) and source reading to locate the code involved
in the reported behaviour:
- Find the code paths that produce the symptom.
- Understand the existing design and any relevant patterns.
- Look for existing features that already solve part of the problem.

Key areas to investigate for this AG Grid component library:
- The Servoy component spec file (`aggrid/datasettable/datasettable.spec` or
  `aggrid/groupingtable/groupingtable.spec`) — defines the component contract
  (model properties, handlers, API methods, types)
- The Angular implementation (`aggrid/projects/nggrids/src/powergrid/` or
  `aggrid/projects/nggrids/src/datagrid/`)
- The shared base class (`aggrid/projects/nggrids/src/nggrid.ts`)
- The component service (`powergrid.service.ts` or `datagrid.service.ts`)
- The AngularJS implementation (`aggrid/datasettable/datasettable.js` or
  `aggrid/groupingtable/groupingtable.js`)
- The `@servoy/public` base classes and utilities
...
```bash
cd /home/gabi/github/aggridcomponents ; git blame -L <start>,<end> "aggrid/<file-path>"
...
- **Is the problem even in Servoy component code?** Or is it user-side (misconfiguration,
  misuse of an API), expected behaviour that's misunderstood, or in a third-party dependency
  (`@servoy/public`, `ag-grid-enterprise`, etc.)?
- **If it is a real bug, is the ticket's proposed approach the right one?** For example,
  the ticket may ask for a new API method when the correct fix is to adjust existing
  property handling — no new API needed.
...
**File location:** `docs/<KEY>-triage.md` (in a `docs/` folder at the repo root).
