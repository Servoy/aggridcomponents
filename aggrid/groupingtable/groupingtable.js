					...
					if (!rowGroupCols.length) { // Grid not grouped anymore
						...
						for (i = 0; i < $scope.model.columns.length; i++) {
							...
							if (autoColumnGroupField && column.columnDef && column.columnDef.checkboxSelection) {
								hideBecauseFieldSharedWithAutoColumnGroup = wasUngrouped || (field && field === autoColumnGroupField); // CHECKME the (field && ...) is an addition of Leo, but doesn't make sense to me
							}
...
//					}
					...
					for (var i = 0; i < groupKeys.length; i++) {
						if (groupKeys[i] === NULL_DISPLAY_VALUE) {
							...
							if (vl) {
								...
								valueListLookupPromises.push(vl.filterList(groupKeys[i]));
				...
				FoundsetServer.prototype.getData = function(request, groupKeys, callback) {
					...
					var rowGroupCols = request.rowGroupCols; // the row group cols, ie the cols that the user has dragged into the 'group by' zone, eg 'Country' and 'Customerid'
				...
				function GroupHashCache() {
					...
					this.clearAll = function() {
						...
						if ($scope.model.hashedFoundsets.length) { // Should not happen, but happens currently cause hashed foundsets never cleared on hide
