angular.module('aggridGroupingtable', ['servoy']).directive('aggridGroupingtable', ['$log', function($log) {
		return {
			restrict: 'E',
			scope: {
				model: '=svyModel'
			...
			controller: function($scope, $element, $attrs) {
				...
				// init the foundset
				var foundset = new FoundSetManager($scope.model.myFoundset);				
				 ...
				 *
				 * */
				function FoundSetManager(foundsetRef) {
				...
				FoundsetDatasource.prototype.getRows = function(params) {
					...
					this.foundsetServer.getData(params.request,
						function successCallback(resultForGrid, lastRow) {
							console.log(lastRow)
							params.successCallback(resultForGrid, foundset.getLastRow());
						});
				};
				...
				FoundsetServer.prototype.getData = function(request, callback) {
					...
					if (request.startRow > 0) {
						var promise = foundset.loadExtraRecordsAsync(CHUNK_SIZE, false);
						promise.then(function (d) {
							console.log(d);
							
							var lastRow = foundset.getLastRow();
							result = foundset.getViewPortData(request.startRow, request.endRow);
						...
						callback(foundset.getViewPortData(0, request.endRow), foundset.getLastRow());
					}
